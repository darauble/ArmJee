...
extern volatile int testvar; // a debugging variable
   ...
   //lets try use the hardware spi class, a bit of a hack
//HardwareSPI myspi(HARDWARESPIPORT);
//spi.begin(SPI_1_125MHZ,MSBFIRST,0); //ok must keep below 2.25Mhz therefore lests start at 1.125Mhz.  It is also big endian (MSBFIRST) and mode 0, ie data on first clock pulse
void rf12_spi_initialize(void);
